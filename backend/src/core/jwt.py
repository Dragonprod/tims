...
def create_access_token(user, secret_key: str) -> str:
    return create_jwt_token(jwt_content=JWTUser(user_id=user.id, is_admin=user.is_admin, role=user.roles).dict(), secret_key=secret_key, expires_delta=timedelta(minutes=JWT_ACCESS_TOKEN_EXPIRE_MINUTES))
